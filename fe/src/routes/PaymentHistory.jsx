      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="md:ml-sidebar-width flex-1 p-8 overflow-y-auto flex flex-col gap-6">
          ...
          {loading ? (
            ...
            <div className="bg-[#121212]/40 border border-white/5 rounded-3xl overflow-hidden shadow-lg">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-[#121212]/50 border-b border-white/5 text-on-surface-variant font-extrabold tracking-wider uppercase text-[10px]">
                ...
                <tbody className="divide-y divide-white/5">
                  {history.map(item => (
                    <tr key={item.id} className="hover:bg-white/5 transition duration-150">
                      <td className="p-4 pl-6 text-on-surface-variant font-medium">{item.date}</td>
                      <td className="p-4 font-mono font-bold text-white">{item.orderId}</td>
                      <td className="p-4 text-on-surface-variant capitalize font-medium">{item.gateway}</td>
                      <td className="p-4 text-primary font-mono font-bold">{item.amount.toLocaleString()} VND</td>
                      <td className="p-4 pr-6 text-center">
                        <span className="text-[10px] px-2.5 py-0.5 rounded-full font-extrabold uppercase bg-primary/10 text-primary border border-primary/20">
                          {item.status}
                        </span>
                      </td>
                    </tr>
